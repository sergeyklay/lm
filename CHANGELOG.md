...
- The chat's status row now says when the model beside it will not come back. A model you chose with `/model` and did not save carries `session only · Ctrl+S` after its name, dim; a model that will come back carries nothing, because being told what you just did yourself is noise. Which model comes back is the one the harness has saved when you have saved one and `LM_MODEL` when you have not, resolved in the same order the launch itself resolves it, so the row and your next launch cannot disagree. The mark is read from the settings file rather than inferred from the event, because the harness announces a model change identically whether you kept it for the session or pressed `Ctrl+S` to keep it for good, so the file is read again on every change and the keystroke clears the mark while you watch. Before, the chat opened on `LM_MODEL` however many times you had picked another model, and nothing on the screen said why.
