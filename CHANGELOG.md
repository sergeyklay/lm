...
- `LM_INSTALL` names the directory `lm` itself is installed in, the one holding its `bin/`, `libexec/` and `docs/`, for a tool that needs `lm`'s own tree rather than the repository it is working on. Only a runner can answer that question: a tool file sits in a project's `tools/`, and a project is not the installation, so a path derived from where the file happens to live names the wrong tree the moment the tool is copied anywhere. Both runners set it, from the command line and from inside the chat alike, so a tool behaves the same however the verb was reached, and a tool that does not need it is unaffected — nothing in the registry changes and a new tool file costs no more than the third one did. `docs/tools.md` publishes it beside the five functions a tool declares.
