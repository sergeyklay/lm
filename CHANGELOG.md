...
- The chat's status row now says when the model beside it will not come back. A model you chose and did not save carries a dim `session only · /model` ahead of its name; a model that will come back carries nothing, because being told what you just did yourself is noise. The mark sits ahead of the name rather than after it, so the name holds the same columns whether the mark is drawn or not and does not shift under your eye. It names the route and not a keystroke: `/model` is what you type from the prompt, and the dialog it opens teaches its own `Ctrl+S` in the one place that key does anything. Which model comes back is the one the harness has saved, provider and model together, when you have saved one, and `LM_MODEL` when you have not, resolved in the same order the launch itself resolves it, so the row and your next launch cannot disagree. The answer is re-derived while the row is drawn, guarded by the settings file's own timestamp and size, rather than taken from the harness's model-change announcement: that announcement is withheld when the model you save is the model already in force, which is the ordinary way to save one, so a row that waited for it would keep the mark on screen after a save that had already been written. Before, the chat opened on `LM_MODEL` however many times you had picked another model, and nothing on the screen said why.

- `lm release` cuts a release in one run. It reads what has accumulated under `## [Unreleased]`, settles the next version, moves those entries under a dated heading of their own and leaves `## [Unreleased]` empty, repairs the link references at the foot of the file, bumps `package.json` and `package-lock.json`, makes one commit and one annotated tag, and pushes the branch and then the tag. Which version it is is a judgement and the arithmetic is not: the answer is `major`, `minor` or `patch` and nothing else, and the number itself is computed from the version already in `package.json`, so a release cannot be cut on a number nothing derived. Name it yourself with `lm release 0.3.0`, and a number that is not the major, minor or patch successor of the current version is refused with the three that are. Everything lands in a single commit, `chore(release): cut <version>`, carrying `package.json`, `package-lock.json` and `CHANGELOG.md` together, because a tree claiming a version whose changelog section does not exist yet is a tree someone can check out and tag. The tag is annotated and reads `lm <version> - <summary>`. Nothing is written until you confirm, once, for the whole cut including the push, and `lm release --dry-run` shows you the commit and the tag it would make and stops. With nothing under `## [Unreleased]` the run exits 3 before the model is called at all and tells you to draft the entries with `lm changelog` first.

- An answer to a confirmation that is neither a yes nor a no is put back to you once, saying what it wants, and a second answer that is still neither exits 7 the way a no does; the question is asked at most twice, never in a loop.
...
- A confirmation now takes `y` or `yes` for a yes and `n`, `no` or an empty line for a no, in either case, so a capitalised answer is no longer read as a refusal.
- `lm commit` now takes the whole uncommitted tree, splits it into one commit per independent change and makes every one of them, so a dirty tree becomes a history without a `git add` of your own. Modified files and untracked ones both go in and `.gitignore` is honoured; the whole grouping is shown once and confirmed once before any commit is made, because a wrong commit boundary is the one thing the validator cannot see; and every commit carries a Conventional Commits message written for the files it holds. Each commit takes exactly the content that was in the tree when the run began, so an edit made while the run is thinking is not swept into it. Before, `lm commit` read the index and only the index, refused an empty one, and wrote a single message for whatever you had staged yourself.

- `--no-stage` is now `lm commit`'s flag and means «take only what is already staged», which is what you want when you staged a subset with `git add -p`. It is typeable at `lm commit` and still at `lm ship`, which goes on carrying it to the verb. `lm ship` no longer stages anything of its own: staging is the verb's now, so a rehearsal of the delivery touches nothing and the flag says what the verb does rather than what the workflow does.

- `lm commit` reports a new exit code, 8, when part of its work landed and the rest did not: a hook rejecting the third commit of three leaves the first two in place, names on stderr which subjects landed and which files are still uncommitted, and exits 8 so that the outcome is told apart from a run where nothing landed at all, which stays 1. Nothing is ever reverted, and running `lm commit` again re-groups what is left. When a hook fails, whether it changed the working tree decides the response: a hook that reformatted the files it was given gets the commit retried once with what it wrote, and a hook that only refused gets no retry, because with the tree unchanged a retry cannot help. `lm ship` stops on 8 and leaves the branch it opened alone rather than deleting the commits that landed on it.
...
- The chat's closing summary is now framed and set out in three headed sections. One border encloses the whole block with a column of padding inside it; `Summary`, `Spend` and `Resume` head the sections in bold; the table's header row has a rule under it; and the figures beside `Session` line up with the model names, which are one shared column rather than a width per section. The command that reopens the session is in the theme's accent rather than in the dimmest ink on the screen, since it is the one row of the block that leaves the screen for a shell, and the frame and the rule are in the theme's border colour. Every figure between them stays uncoloured: a colour on a count is a claim about the count, and a session that ran nothing still reads `0 ran, 0 failed` plainly, with no share over an empty denominator and nothing colouring it as a failure. The frame costs the sides and the padding out of the terminal's width, and everything inside it is sized against what it leaves rather than against the terminal, so an elided model identifier and a broken resume command stay inside the border instead of crossing it. It is drawn only where the terminal can hold it: the interior has to clear the same floor of twenty columns the contents were already given, and every row has to fit that interior, which an identifier and the table's four counted columns cannot always be made to do. A terminal failing either test gets the block unframed, exactly as it looked before.
...
- `lm commit` no longer puts a file name where the change's area belongs. The scopes it offers the model are built from the staged paths and were listed in alphabetical order, which put `CHANGELOG.md` at the head of the list on every change that touches the changelog: two commits here went out as `feat(CHANGELOG)` for changes to the chat's closing block. The list is now ordered by how many changed lines each candidate owns, so the area carrying the change leads it and a one-line changelog bullet trails it. Added and deleted lines both count, a binary file counts as none, a candidate collects a file's lines once however many ways that path spells it, and a renamed file counts for both the name it left and the name it took. The cap of twenty-five now keeps the twenty-five candidates carrying the most of the diff rather than the first twenty-five in the alphabet. Which candidates are on offer is otherwise unchanged, so a file with no extension still contributes its directory and neither its own name nor a stem of one.
